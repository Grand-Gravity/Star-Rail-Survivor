...
int experience = 0, span = 3;//经验，升级所需经验
int level = 0;
int enemy_hp = 100, enemy_atk = 100, enemy_speed = 4;
bool option = false;//是否正处于选择强化界面
...
void DrawPlayerUI(int score,Player* p) {
	static TCHAR text[64];
	_stprintf_s(text, _T("生命：%d/%d   攻击：%d   分数：%d   经验：%d/%d   等级：%d"), p->hp, p->hp_max, p->atk, score, experience, (span + level), level);
